...
export const alumnosessionGuard: CanActivateFn = (route, state) => {
  ...
  // Verificar si el token existe para profesor o admin o alumno, si no existe redirigir a la ruta home
  if(token){
    const decodedToken: any = jwtHelper.decodeToken(token);
    const role: string = decodedToken?.rol || '';
    if (role === "profesor" || role === "admin" || role === "alumno" ){
